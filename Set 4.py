...
def dec_to_bin(number):
    ...
    reversed_bin_list = list(reversed_bin)
    bin = "".join(reversed_bin_list[::-1])
...
def telephone_cipher(message):
    ...
    for i in message:
        if next_index < len(message):
            if encoder_dict[i][0] == encoder_dict[message[next_index]][0]:
                numerical_string += encoder_dict[i] + "_"    
            else:
                numerical_string += encoder_dict[i]
            next_index += 1
        else:
            numerical_string += encoder_dict[i]
...
def telephone_decipher(telephone_string):
    ...
    grouped_num_list = [sublist for sublist in grouped_list if sublist != ["_"]]
    
    for item in grouped_num_list:
        deciphered += decipher_dict[item[0]]
